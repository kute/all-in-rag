...
- 步骤1：多表征索引（Multi-representation Indexing）
原始文档通过大语言模型（LLM）处理，生成检索优化的摘要。这些摘要和元数据被存储在向量存储（Vectorstore）和文档存储（Docstore）中。这样做的目的是创建更适合检索的文档表征形式，比如摘要等，以提高检索效果。
- 步骤2：查询转换（Query Transformation）
当用户提出问题时，系统会对原始查询进行优化处理。通过大语言模型重新表述查询、生成子查询、或将对话历史压缩成查询等方式，生成检索优化的问题。这一步确保查询更精确、更适合从数据库中检索相关信息。
- 步骤3：查询构建（Query Construction）
系统将自然语言查询转换为结构化的查询语言，比如添加元数据过滤器、生成SQL查询等。这使得系统能够更精确地从向量存储和文档存储中检索信息。
