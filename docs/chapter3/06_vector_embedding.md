...
- Zero-shot - 零样本学习能力,指模型在没有针对特定任务进行微调的情况下直接执行任务的能力,反映模型的开箱即用性和泛化能力。
- Memory Usage (MB) - 模型运行时占用的内存大小(单位:兆字节),影响部署成本和在资源受限环境中的可用性。
- Number of Parameters (B) - 模型包含的可训练参数总量(单位:十亿),参数越多通常性能越强但推理速度越慢,是衡量模型规模的关键指标。
- Embedding Dimensions - 输出向量的维度数(如512、1024),维度越高可编码更丰富的语义信息,但存储和计算成本也相应增加。
- Max Tokens - 模型能处理的最大输入文本长度(以token为单位),超出此长度的文本会被截断,影响长文档处理效果。
- Mean (Task) - 所有具体评测任务得分的算术平均值,是衡量模型综合性能的最核心指标,通常用于排行榜排名。
- Mean (TaskType) - 按任务类型(如检索类、分类类)分组后各类型得分的平均值,反映模型在不同类型任务上的均衡表现。
- Bitext Mining - 双语文本挖掘任务得分,评估模型在多语言平行语料中识别互译句对的能力,对跨语言检索和机器翻译应用至关重要。
- Classification - 文本分类任务的平均得分,评估embedding作为分类器特征输入时的质量,涵盖情感分析、主题分类等场景。
- Clustering - 聚类任务得分,测试模型将语义相似文本自动分组的能力,应用于文档组织、主题发现等无监督学习场景。
- Instruction Reranking - 指令重排序任务得分,专门评估模型对指令类文本(如任务描述、提示词)的相关性排序能力,对构建指令跟随系统很有价值。
- Multilabel Classification - 多标签分类得分,评估模型处理一个文本同时属于多个非互斥类别的能力,比单标签分类更具挑战性。
- Pair Classification - 文本对分类得分,评估模型判断两个文本间关系(如是否为释义、是否重复)的能力,典型应用包括问题重复检测和文本蕴含识别。
- Reranking - 重排序任务得分,评估模型对初步检索结果进行精细化重新排序的能力,常用于两阶段检索系统的第二阶段优化。
- Retrieval - 信息检索任务得分,评估模型根据查询从大规模文档库中找到相关文档的能力,是embedding模型最核心和常用的应用场景。
- STS - 语义文本相似度得分,评估模型预测两个句子语义相似程度(通常0-5分)的准确性,通过与人工标注相似度的相关系数来衡量。
